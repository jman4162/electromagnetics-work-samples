...
x_arr = linspace(0.0001, 1, 11);
z_arr = linspace(0.0001, 1, 45);

ex_Einc_re_arr = zeros(length(x_arr), length(z_arr));
...
for xx = 1:length(x_arr)
    ...
    ex_Einc_re_arr(xx, zz) = ex_Einc_re;
...
figure;
imagesc(x_arr, z_arr, ex_Einc_re_arr')
xlabel('x (m)')
ylabel('z (m)')
